...
import javafxapplication1.service.impl.*;
...
public class FXMLController implements Initializable {
     ...
     */
    public static lcfs lc;
    public static Fcfs fc;
    public static Roben rb;
    public static Priority pri;
    public static ShReFi shor;
    ...
    File f1;
    public static Process[] arr;
    public static ArrayList<Process> ar;
    @FXML
    private void handlechange(ActionEvent event) throws IOException {
        Parent gotonew = FXMLLoader.load(getClass().getResource("fxml/details.fxml"));
    ...
    void rrb(ActionEvent event) throws IOException {
        ...
        int rocont = new Integer(roben.getText());
        int time= new Integer(timelimit.getText());
        rb = new Roben(arr, rocont,time);
        who=0;
        Roundchart rond = new Roundchart();
        rond.start(new Stage());
                FXMLLoader gotonew = new FXMLLoader(getClass().getResource("fxml/details.fxml"));

        //FXMLLoader  gotonew = FXMLLoader.load(getClass().getResource("details.fxml"));
                Parent root1 = gotonew.load();
...
//        current.setScene(go);
 FXMLLoader gotonew = new FXMLLoader(getClass().getResource("fxml/details.fxml"));
    ...
    void prio(ActionEvent event) throws IOException { // priority function 
        ...
        who = 1 ; 
            for (int i = 0; i < arr.length; i++) {
           for (int j = 0; j < pri.processResultList[i].workingIntervals.size(); j++) {
               System.out.println(pri.processResultList[i].PIC+"==  work from  "+pri.processResultList[i].workingIntervals.get(j).getKey()+" to "+pri.processResultList[i].workingIntervals.get(j).getValue());
           } 
        }
                FXMLLoader gotonew = new FXMLLoader(getClass().getResource("fxml/details.fxml"));
                Parent root1 = gotonew.load();
