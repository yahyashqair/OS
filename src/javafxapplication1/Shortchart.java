...
public class Shortchart extends Application{
    ...
    @Override public void start(Stage stage) {
        ...
        for(int i = 0 ; i < serie.length; i ++ ){// i indecate to process
            ...
            serie[i].getData().add(new XYChart.Data(rec.arr[i].waitlist.get(j).getKey(), proc[i], new GanttChart.ExtraData( rec.arr[i].waitlist.get(j).getValue()-rec.arr[i].waitlist.get(j).getKey(), "status-blue")));
            ...
            serie[i].getData().add(new XYChart.Data(rec.arr[i].worklist.get(j).getKey(), proc[i], new GanttChart.ExtraData( rec.arr[i].worklist.get(j).getValue()-rec.arr[i].worklist.get(j).getKey(), "status-green")));
            }
